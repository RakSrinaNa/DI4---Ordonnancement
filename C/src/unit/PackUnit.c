...
void packUnit()
{
	...
	if(!unit_uintArrayEquals(correctOrder1, pack->tasks, 1))
	...
	if(!unit_uintArrayEquals(correctOrder2, pack->tasks, 3))
	...
	if(!unit_uintArrayEquals(correctOrder3, pack->tasks, 3))
	...
	if(!unit_uintArrayEquals(correctOrder4, pack->tasks, 3))
		unit_error("Pack 10: Bad move forward");
	
	pack_moveDelivery(pack, 0, 0);
	unsigned int correctOrder5[] = {0, 2, 1};
	if(!unit_uintArrayEquals(correctOrder5, pack->tasks, 3))
